...
class ReportService {
  ...
  async generateReportContent(target, vulnerabilities, reconResults, template) {
    const ALLOWED_TEMPLATES = ['basic', 'detailed', 'executive', 'technical'];
    ...
    if (fs.existsSync(resolvedPath)) {
      try {
        let templateContent = fs.readFileSync(resolvedPath, 'utf8');
...
Generated by Bug Bounty System
  ...
  async exportReport(reportId, format) {
    ...
    const safeName = (report.target_name || 'report').replace(/[^a-zA-Z0-9._-]/g, '_');
    let contentType, filename, content;

    switch (format.toLowerCase()) {
      case 'json':
        contentType = 'application/json';
        filename = `${safeName}-report.json`;
        content = JSON.stringify(report, null, 2);
        break;
      case 'txt':
        contentType = 'text/plain';
        filename = `${safeName}-report.txt`;
        content = report.content;
        break;
      case 'pdf':
        // For now, return markdown
        contentType = 'text/markdown';
        filename = `${safeName}-report.md`;
        content = report.content;
        break;
      default:
        contentType = 'text/markdown';
